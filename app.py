...
from analysis import transfer_suggestions
...
def register_route():
    ...
    if form.validate_on_submit():
        ...
        # redirect to the next part of creating the tem table, and pass along the new user's id to associate it with the new user -- this table will hold all the info about
...
# create the new user's team table with all pertinent info
# this table of course points back to the new user and will hold the new user's
# team info in our database for future fetching
...
# LOGGING IN ###################################################################
# logging the user in with email and password
...
def login_user():
    ...
    if form.validate_on_submit():
        ...
        # if login is successful and user MADE transfers
        if user:
            if transfer_check > 0:
                session['user_id'] = user.id
                team = Team.query.get_or_404(user.id)
                # update team instance and pass it along to make a new version of the user's team
                user_instance = team.team_instance + 1
                return redirect(f'/user/instance/{user.id}/{user_instance}')
            else:
                session['user_id'] = user.id
...
# transfers were made
# create a new version of user's team and update user's team instances
@app.route('/user/instance/<int:user_id>/<int:user_instance>', methods=["GET","POST"])
def update_userS_team_version(user_id, user_instance):
    # creating a nwe version of the user's team
    user = Users.query.get_or_404(user_id)
    user.team_instance = user.team_instance + 1
    db.session.add(user)
    db.session.commit()
    ...
    db.session.commit()
    session['user_id'] = user_id
    flash('Transfers added and team updated!')
    return redirect(f'/user/{user_id}')
...
def user_roster(user_id, sort_num):
    ...
    return render_template('user_roster.html', player=player, roster=roster, sort_num=sort_num)

# transfer suggestions
@app.route('/user/transfer_suggestions/<int:user_id>')
def user_transfer_suggestions(user_id):
    player = Team.query.get_or_404(user_id)
    return render_template('user_transfer_suggestions.html', player=player, transfer_suggestions=transfer_suggestions)

# league tables
@app.route('/user/league_tables/<int:user_id>')
def user_league_tables(user_id):
    player = Team.query.get_or_404(user_id)
    return render_template('user_league_tables.html', player=player, user_profile=user_profile)
